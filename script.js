const translations = {
    ...
    ko: {
        ...
        exhibitions: "전시회",
        exhibitionsLegend:
            "아래 항목은 대부분 독일어로 작성되어 있습니다. 용어: 그룹전, 개인전, 온라인 전시, 오픈 스튜디오.",
        education: "학력",
        contactTitle: "연락처",
        contactEmail: "이메일",
        contactWebsite: "사이트",
...
function updateLanguageBlocks(lang) {
    document.querySelectorAll(".lang-block").forEach((block) => {
        block.style.display =
            block.getAttribute("data-lang-block") === lang ? "" : "none";
    });
}
...
/* UPDATED: Lightbox with info panel */
function setupLightbox() {
    ...
    lightbox.innerHTML = `
        ...
        <div class="lightbox-content">
            ...
            <div class="lightbox-panel" aria-live="polite">
    ...
    function openLightboxFromImg(img) {
        ...
        const parts = [];
        if (year) parts.push(`<div><strong>Year:</strong> ${year}</div>`);
        if (medium) parts.push(`<div><strong>Medium:</strong> ${medium}</div>`);
        if (size) parts.push(`<div><strong>Size:</strong> ${size}</div>`);
        if (note) parts.push(`<div>${note}</div>`);
